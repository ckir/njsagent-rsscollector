<?php
namespace Rss\Util\Contrib\Nlp\Stemmers\en\Stemmer;
...
class Stemmer {
		...
		if (substr ( $word, - 3 ) == 'eed') {
			...
			if (preg_match ( '/([aeiou]|[^aeiou]y).*(ed|ing)$/', $word )) { // vowel in stem
				...
				if (substr ( $word, - 2 ) == 'at' || substr ( $word, - 2 ) == 'bl' || substr ( $word, - 2 ) == 'iz') {
					...
					if ($this->is_consonant ( $word, - 1 ) && $last_char == $next_to_last && $last_char != 'l' && $last_char != 's' && $last_char != 'z') {
						$word = substr ( $word, 0, - 1 );
					} else {
	 ...
	 *
	 * @param string $word
	 *        	Word to check and modify
	 * @param string $suffix
	 *        	Suffix to look for
	 * @param string $replace
	 *        	Suffix replacement
	 * @param integer $m
	 *        	Word measure value that the word must be greater
	 *        	than to replace
	 * @access private
	 * @return boolean
	 */
